...
from packages.components.sitting_down import ComponentSittingDown


class TestClass2(TestClass):
    def test_sitting_down_component(self):
        self.init_things()
        self.generate_fresh_room()
        global_textadv.player_ref = new_object(Player)
        self.fresh_room.add_to_room(global_textadv.player_ref)
        global_textadv.player_ref.add_component(ComponentSittingDown, {})
        laying_component: ComponentSittingDown = global_textadv.player_ref.get_component(
            ComponentSittingDown)
        ...
        assert not global_textadv.player_ref.get_component(
            ComponentSittingDown)
