...
class Player(PhysObj):
    ...
    def __init__(self) -> None:
        ...
        self.register_event(
            self, events.EVENT_PHYSOBJ_ENTERED_ROOM, self.on_enter_room)
    ...
    def look_around_room(self, room_to_look: "room.Room" = None, entering_room: bool = False):
        ...
        event_retval: str = self.send_event(
            room_to_look, EVENT_BASEOBJ_PRINT_DESCRIPTION)
    ...
    def parse_text(self, text_to_parse: str) -> bool:
        ...
        for command_tuple in command_tuples:
            ...
            # Doesn't count "read" in "read thing" due to start-at-0 memes
            words_used_list = list(range(words_used + 1))
            ...
            for i in range(len(verb.expected_args)):
                ...
                # Todo here: add a way  # What did he mean by this
    ...
    def arg_check(self, verb: "Verb", arg_list: list[str], verb_argument_pos: int) -> tuple[(PhysObj | list[PhysObj]), int]:
        ...
        for i in reverse_list:
            ...
            for i2 in range(i + 1):
                ...
                nearby_objects: list[PhysObj] = self.find_nearby_objects_by_name(
                    argument)
    ...
    def check_command(self, word_list: list[str]) -> list[tuple[(PhysObj | list[PhysObj]), int, "Verb"]]:
        ...
        for i in word_list_reversed:
            command = ""
            for i2 in range(i + 1):
                # Starts with "a b c d e", goes to "a b c d", etc.
                command += ((" " if i2 > 0 else "") + word_list[i2])
            ...
            inventory_list = self.send_event(
                self, EVENT_INVENTORY_GET_CONTENTS)
            if inventory_list:
    ...
    # feels odd having this on player, may change if i find smth better? #Maybe make it on baseobj and override?
    def find_nearby_objects_by_name(self, obj_name: str) -> list[PhysObj]:
        ...
        if inventory_list:
            for item in inventory_list:
                ...
                extra_items: list[PhysObj] = self.send_event(
                    item, EVENT_PLAYER_FIND_CONTENTS) or []
        ...
        for object in self.current_room.contents:
            ...
            extra_objects: list[PhysObj] = self.send_event(
                object, EVENT_PLAYER_FIND_CONTENTS) or []
