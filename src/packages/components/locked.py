...
class ComponentLocked(Component):
    ...
    def __init__(self, args_dict = dict[str]) -> None:
        ...
        self.unlock_message: str = self.arg_set(args_dict, "unlock_message", str) or "You unlock the thing."
        # A message to show when the parent is examined. Will not trigger if message is empty
        self.examine_message: str = self.arg_set(args_dict, "examine_message", str)
    ...
    def attach_to_parent(self, object_to_attach: BaseObj) -> bool:
        ...
        self.register_event(object_to_attach, EVENT_LOCK_ATTEMPT_UNLOCK, self.unlock)
    ...
    def detach_from_parent(self):
        ...
        if physobj_parent:
            physobj_parent.remove_trait(TRAIT_LOCKED, f"{self.id}_{self.lock_id}")
