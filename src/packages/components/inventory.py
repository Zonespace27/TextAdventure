...
class ComponentInventory(Component):
    ...
    def attach_to_parent(self, object_to_attach: BaseObj) -> bool:
        ...
        self.register_event(
            object_to_attach, EVENT_INVENTORY_ADD_OBJECT, self.on_attempt_object_add)
        self.register_event(
            object_to_attach, EVENT_INVENTORY_REMOVE_OBJECT, self.on_attempt_object_remove)
        self.register_event(object_to_attach, EVENT_INVENTORY_GET_CONTENTS,
                            self.return_inventory_contents)  # We commit a mild amount of sin
        self.register_event(
            object_to_attach, EVENT_VERB_CHECK_INVENTORY, self.check_inventory)
        ...
        if (len(self.inventory)):
            ...
            for index, inv_object in enumerate(self.inventory):
                contents += inv_object.name + \
                    ("\n" if (self.inventory[index] == self.inventory[len(
                        self.inventory) - 1]) else ", ")

            contents += f"in your inventory, it looks like it could hold {remaining_space} more thing" + (
